...
def  SSQ_PR_samples  ( log_file, arr_counts , arr_genes, arr_cells, n_sample_pairs ):
  ...
  arr_sample_size = df_cell_samples.sum()
  # print (  '\n arr_sample_size: \n', arr_sample_size , file=log_file )    
  
  df_sample_sizes = pd.DataFrame ( index = list( range ( 2*n_sample_pairs ) ), data = arr_sample_size, columns=['sample_size'] )     
 

  # print (  '\n exiting SSQ_PR_samples ', file=log_file )   

  return  ( df_cell_samples, df_SSQ_PR_samples, df_sample_sizes )    
...
def Sg_analysis ( log_file, df_counts_in, nz_min=50, n_sample_pairs=20 ): 
  ...
  df_cell_samples, df_SSQ_PR_samples, df_sample_sizes = SSQ_PR_samples  ( log_file, arr_counts_nz, arr_genes_nz, arr_cells_nz, n_sample_pairs )
  
  df_SSQ_PR_all_and_samples = pd.concat ( [ df_nz_cells, df_SSQ_PR, df_SSQ_PR_samples ], axis=1 ) 
  
  return_dict = { 'df_cell_samples':df_cell_samples, 'df_SSQ_PR_all_and_samples':df_SSQ_PR_all_and_samples, 'df_sample_sizes':df_sample_sizes }
...
def  cell_max_contribution_to_SSQ_PR_samples ( log_file, df_counts, df_cell_samples ): 
  ...
  df_max_contribution_list = []
  df_gene_max_contribution_list = []
  ...
  for sample in  sample_list:
    ...
    arr_cell_max_contribution = np.ravel ( ( arr_SSQ_gc.max( axis=0 ) ).todense() )
    arr_cell_arg_max_contribution =  arr_SSQ_gc.argmax( axis=0 )   
    arr_gene_max_contribution = np.ravel ( arr_genes_sample_nz[ arr_cell_arg_max_contribution ] )
  
    df_sample_max_contribution = pd.DataFrame ( index=arr_cells_sample_nz, data=arr_cell_max_contribution, columns = [ sample ] )
    df_max_contribution_list.append ( df_sample_max_contribution )
  
    df_sample_gene_max_contribution = pd.DataFrame ( index=arr_cells_sample_nz, data=arr_gene_max_contribution, columns = [ sample ] )
    df_gene_max_contribution_list.append ( df_sample_gene_max_contribution ) 
    # pdline( log_file )    

  df_max_contribution = pd.concat ( df_max_contribution_list, axis= 1 )
  # print (  '\n\n df_max_contribution \n', df_max_contribution , file=log_file )
   
  df_gene_max_contribution = pd.concat ( df_gene_max_contribution_list, axis= 1 )
  # print (  '\n\n df_gene_max_contribution \n', df_gene_max_contribution , file=log_file )


   
  cell_list = df_gene_max_contribution.index.values.tolist()
  cells_contribution_list = []
  cells_gene_list = []
  
  for cell in cell_list:
    arr_max_contribution = df_max_contribution.loc[ cell ].values
    arr_gene_max_contribution = df_gene_max_contribution.loc[ cell ].values

    arr_max_contribution_notNA = arr_max_contribution[~np.isnan(arr_max_contribution)]  
    arr_gene_max_contribution_notNA = arr_gene_max_contribution[~np.isnan(arr_max_contribution)]   
  
    sample_max_contribution = arr_max_contribution_notNA.argmax()   
    max_contribution = arr_max_contribution_notNA[ sample_max_contribution ]
    gene_max_contribution = arr_gene_max_contribution_notNA[ sample_max_contribution ]  

    cells_contribution_list.append ( max_contribution )
    cells_gene_list.append ( gene_max_contribution )  

  df_cell_max_contribution = pd.DataFrame ( index=cell_list, data= {'gene':cells_gene_list, 'contribution':cells_contribution_list } )
  # print (  '\n\n df_cell_max_contribution \n', df_cell_max_contribution, file=log_file ) 
  # print (  '\n\n df_cell_max_contribution[contribution].describe \n', df_cell_max_contribution['contribution'].describe ( percentiles=pctl_list ), file=log_file )
  
  return  df_cell_max_contribution

######################################################################################         

def outliers ( log_file, df_values, value ):
...
def  identify_cell_outliers ( log_file, df_counts, df_cell_samples ): 
  ...
  df_cell_max_contribution = cell_max_contribution_to_SSQ_PR_samples ( log_file, df_counts, df_cell_samples ) 
  dict_outliers = outliers ( log_file, df_cell_max_contribution, 'contribution' )
  ...
  return  { 'df_cell_max_contribution':df_cell_max_contribution, 'df_cells_drop':df_cells_drop, 'cells_retain_list':cells_retain_list } 
...
def  instability_ratios  ( log_file, df_SSQ_PR_samples ):
  ...
  df_IR['instabilty_ratio'] = df_IR['max'] / df_IR['min']
  # print (  '\n\n df_IR \n', df_IR , file=log_file )
...
def  identify_gene_outliers ( log_file, df_counts_X_cell_OL, df_cell_samples ):
  ...
  ### df_cell_samples_out not used here
  ...
  dict_outliers = outliers ( log_file, df_IR, 'instabilty_ratio' )
