...
############################################################## 
#                                                            #
#  genes_HV_in_all_samples_csc.py                            # 
#                                                            #
############################################################## 
...
out_name = "genes_HV_in_all_samples_csc"

logfile_txt = out_name + ".txt"
plot_pdf =  out_name + ".pdf"
out_pkl =  out_name +  ".pkl"

Sg_filtered_dict_filtered_pkl =  "dict_Sg_stats_and_samples_filtered_counts_csc.pkl"
...
#### pickle inputs
Sg_filtered_dict_filtered_dsn = data_path / Sg_filtered_dict_filtered_pkl
...
f = open( Sg_filtered_dict_filtered_dsn, 'rb' )    
...
df_impact_of_filtering = dict_impact_of_filtering [ 'df_stats_filtered' ]
print (  '\n\n df_impact_of_filtering \n', df_impact_of_filtering , file=logfile )

pdline ( logfile )


df_sample_sizes = Sg_filtered_dict['df_sample_sizes']
print (  '\n\n df_sample_sizes \n', df_sample_sizes , file=logfile )
 
df_SSQ_PR_samples = Sg_filtered_dict['df_SSQ_PR_all_and_samples'].drop ( columns=['nz_cells', 'S_g'] ) 
print (  '\n\n df_SSQ_PR_samples \n', df_SSQ_PR_samples , file=logfile )

df_MSSQ_PR_samples = df_SSQ_PR_samples.div (  df_sample_sizes['sample_size'], axis=1 )
print (  '\n\n df_MSSQ_PR_samples \n', df_MSSQ_PR_samples , file=logfile )


df_nz_Sg =  Sg_filtered_dict['df_SSQ_PR_all_and_samples'][[ 'nz_cells', 'S_g' ]]
n_cells = len ( Sg_filtered_dict['df_cell_samples'] )
df_Mg = df_nz_Sg[['S_g']].rename ( columns={'S_g':'M_g'} ) / n_cells

df_gene_stats = pd.concat ( [ df_nz_Sg[['nz_cells']], df_Mg, df_MSSQ_PR_samples], axis=1 )
print (  '\n\n df_gene_stats \n', df_gene_stats , file=logfile )

pdline( logfile )
 

df_gene_stats.sort_values ( ['M_g'], ascending=False, inplace=True )
print (  '\n\n df_gene_stats \n', df_gene_stats , file=logfile )
print (  '\n\n df_gene_stats.describe \n', df_gene_stats[['M_g']].describe ( percentiles=pctl_list ), file=logfile )

df_Mg_rank = df_gene_stats.drop ( columns=[ 'nz_cells' ] ).rank ( ascending=False )
...
df_merg = pd.concat ( [ df_gene_stats [[ 'nz_cells', 'M_g' ]], df_HV_all_bool ], axis=1 )
print (  '\n\n df_merg \n', df_merg , file=logfile )


df_genes_analysis_subset = df_merg[ df_merg['HV_all'] ].drop ( columns=['HV_all'] )
...
df_plot_data = pd.concat ( [ df_merg, df_impact_of_filtering[['instabilty_ratio']] ], axis=1 )
...
print ( '\n\n program  genes_HV_in_all_samples_csc.py  elapsed time (seconds): ',  f"{ elapsed:.1f}" , file=logfile )
