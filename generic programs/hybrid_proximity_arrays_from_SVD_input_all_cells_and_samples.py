...
#### https://www.geeksforgeeks.org/how-to-convert-a-dictionary-into-a-numpy-array/  -- did not follow completely



###########################################################################################
#                                                                                         #        
#      hybrid_proximity_arrays_from_SVD_input_all_cells_and_samples.py                    #
#                                                                                         #   
###########################################################################################
...
in_X_EOL_name = "exclude_Euclidean_outliers"
in_residuals_samples_name = "SVD_residuals_X_Euclidean_outliers_samples"


out_name = "hybrid_proximity_arrays_from_SVD_input_all_cells_and_samples"




logfile_txt = "calculate_" + out_name + ".txt"
dict_proximities_pkl = "dict_" + out_name + ".pkl"
...
def  proximity_array ( df_coordinates, n_neighbors ):     
  ...
  print ('\n\n in function proximity_array - X.shape:  ', n_cells ,file=logfile )
  ...
  arr_proximity_csr =  sparse.coo_array( ( arr_data, ( arr_row, arr_col ) ), shape=( n_cells, n_cells ) ).tocsr()
  del arr_row, arr_col, arr_data  
 
  tuple_find = sparse.find ( arr_proximity_csr )
  arr_nz_values = tuple_find[2]    
  print ( ' number of entries in arr_proximity_csr: ',  arr_nz_values.shape, file=logfile )    

 
  return { 'cell_list':cell_list, 'arr_proximity_csr': arr_proximity_csr } 
...
dict_proximities_all_cells =  proximity_array ( df_S_Vt_X_outliers, n_neighbors_max )
pdline( logfile, char='=' )
    
################  

dict_proximities_samples = {}
...
for sample in sample_list:
  ...
  dict_sample_proximities = proximity_array( dict_sample_df_S_Vt, n_neighbors_max )    
    
  dict_proximities_samples[ sample ] = dict_sample_proximities  
  pdline ( logfile )
...
print ( '\n\n program  hybrid_proximity_arrays_from_SVD_input_all_cells_and_samples.py  elapsed time (seconds): ',  f"{ elapsed:.1f}" , file=logfile )

  

dict_out = {'dict_proximities_all_cells':dict_proximities_all_cells, 'dict_proximities_samples':dict_proximities_samples }

f = open( dict_proximities_dsn, 'wb' )    
