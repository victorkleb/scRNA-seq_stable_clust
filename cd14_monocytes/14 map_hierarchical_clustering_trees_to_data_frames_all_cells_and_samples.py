...
in_clustering_trees = "NJW_spectral_hierarchical_clustering_trees_all_cells_and_samples_seq_" +  str ( sequence ) 

out_name = "map_hierarchical_clustering_trees_to_data_frames_all_cells_and_samples_seq_" +  str ( sequence ) 
...
dict_clustering_trees_pkl = "dict_" + in_clustering_trees + ".pkl"
...
def  node_list_to_clusters ( H_clustering_tree_parm, node_list ):
  ...
  for node in  node_list:  
    cell_list = H_clustering_tree_parm[ node ] ['cell_list'] 
...
def  map_tree_to_df ( H_clustering_tree ):  
  dict_clusterings_nodes = {}
  df_clusterings_list = []
  df_clusters_sizes_list = []   


  depth_list = list ( set ( [ node[0] for  node in  list ( H_clustering_tree.keys() ) ] ) )
  depth_list.sort()
  ...
  for depth in depth_list:
    node_list = [ node for  node in  list ( H_clustering_tree.keys() ) if  ( node[0] == depth ) ]
  ...
  for depth in depth_list[1:]:
    ...
    for  node in   node_list: 
      depth_distance = H_clustering_tree[ node ] ['cum_depth_Heuristic_distance']
      depth = node[0]
      terminal = H_clustering_tree[ node ] ['terminal'] 
  ...
  for depth_distance in  list_depth_distances[ :max_n_clusters -1 ] :
    ...
    return_dict = node_list_to_clusters ( H_clustering_tree, clustering_node_list )
...
f = open( dict_clustering_trees_dsn, 'rb' )    
dict_H_cluster_trees = pickle.load(f)           
f.close() 


H_clustering_tree_all_cells = dict_H_cluster_trees['H_clustering_tree_all_cells'] 

dict_H_cluster_trees_samples = dict_H_cluster_trees['dict_H_cluster_trees_samples'] 
 
sample_list = list ( dict_H_cluster_trees_samples.keys() )  
sample_list.sort()
print (  '\n\n  sample_list:  ', sample_list,  file=logfile ) 


del dict_H_cluster_trees

pdline ( logfile ) 

############### all cells 
  
return_dict = map_tree_to_df ( H_clustering_tree_all_cells )
...
for sample in sample_list:
  ...
  H_clustering_tree	=  dict_H_cluster_trees_samples [ sample ]


  return_dict = map_tree_to_df ( H_clustering_tree )
...
print ( '\n\n program  map_hierarchical_clustering_trees_to_data_frames_all_cells_and_samples.py  elapsed time (seconds): ',  f"{ elapsed:.1f}" , file=logfile )
