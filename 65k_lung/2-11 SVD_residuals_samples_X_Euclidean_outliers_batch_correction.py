...
##########################################################################
#                                                                        #
#    SVD_residuals_samples_X_Euclidean_outliers_batch_correction.py      # 
#                                                                        #
##########################################################################
...
dict_Sg_dict_in_pkl =   "dict_of_dicts_Sg_stats_and_samples_batch_correction_seq_"   + str ( sequence ) + ".pkl"
...
dict_Sg_stats_and_samples_dict_batch = pickle.load(f)           
f.close()       


samples_list = dict_Sg_stats_and_samples_dict_batch[ batches_list[0] ] ['df_cell_samples'].columns.tolist()
...
for sample in samples_list:
  df_PR_sample_batch_list = []
  
  for batch in batches_list:
    print ( '\n sample: ', sample )         
    print ( ' batch: ', batch, )          
    print ( '\n sample: ', sample, file=logfile )         
    print ( ' batch: ', batch, file=logfile )    

    df_counts = dict_df_counts[ batch ]
    print (  '\n\n df_counts \n', df_counts , file=logfile )
    print (  '\n\n df_counts.sparse.density:  ', df_counts.sparse.density, file=logfile )
  
    df_cell_samples =  dict_Sg_stats_and_samples_dict_batch[ batch ] ['df_cell_samples']
    ...
    df_PR_sample_batch = compute_pearson_residuals  ( logfile, arr_counts_sample, arr_genes_sample, arr_cells_sample,  arr_gene_subset )    
    print ( '\n\n df_PR_sample_batch: \n', df_PR_sample_batch, file=logfile )    

    df_PR_sample_batch_list.append ( df_PR_sample_batch ) 

  df_PR_sample = pd.concat ( df_PR_sample_batch_list, axis=1 ).fillna(0)
  print ( '\n\n df_PR_sample: \n', df_PR_sample, file=logfile )    
   
  df_S_Vt = S_Vt ( df_PR_sample, estim_ncp_dim )
  ...
  df_SVD_residuals_XOL_sample = df_S_Vt [ sample_X_EOL_cell_list ]
  print ( '\n\n df_SVD_residuals_XOL_sample: \n ', df_SVD_residuals_XOL_sample, file=logfile )  
      

  dict_SVD_residuals_samples[ sample ] = df_SVD_residuals_XOL_sample
  pdline ( logfile, char='=' )
...
print ( '\n\n program  SVD_residuals_samples_X_Euclidean_outliers_batch_correction.py  elapsed time (seconds): ',  f"{ elapsed:.1f}" , file=logfile )
